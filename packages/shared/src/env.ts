...
// Hardened profile: telemetry must be explicitly opted in.
// An injected endpoint alone must never turn collection on.
export const ZCODE_TELEMETRY_ENABLED: boolean =
  typeof process !== "undefined" &&
  ["1", "true", "yes", "on", "enabled"].includes(
    (process.env.ZCODE_ENABLE_TELEMETRY ?? "").trim().toLowerCase(),
  );
